...
$product_for_sku       = $bundled_item->get_product();
$set_stock_status = '';
if($product_for_sku->get_stock_status() == 'instock'){
	$set_stock_status = 'in stock';
}else if($product_for_sku->get_stock_status() == 'onbackorder' ){
	$set_stock_status = 'on back order';
}else{
	$set_stock_status = 'out of stock';
}

?><h4 class="bundled_product_title product_title"><?php
		$title = '<span class="bundled_product_title_inner">' . WC_PB_Helpers::format_product_shop_title( $title, $quantity, '', $title_suffix ). '</span></br><span> SKU: '. $product_for_sku->get_sku().'</span></br><span class="stock_status">'. $set_stock_status.'</span>';
