...
use UpsFreeVendor\WPDesk\WooCommerceShipping\ShippingMethod\HasCollectionPointFlatRate;
...
use UpsFreeVendor\WPDesk\WooCommerceShipping\ShippingMethod\RateMethod\FlatRateRateMethod\CollectionPointFlatRateRateMethod;
use UpsFreeVendor\WPDesk\WooCommerceShipping\ShippingMethod\RateMethod\Standard\StandardServiceRateMethod;
use UpsFreeVendor\WPDesk\WooCommerceShipping\ShippingMethod\Traits\DeliveryDatesTrait;
use UpsFreeVendor\WPDesk\WooCommerceShipping\ShippingMethod\Traits\MetaDataTrait;
use UpsFreeVendor\WPDesk\WooCommerceShipping\ShippingMethod\Traits\RateWithRateMethodsTrait;
use UpsFreeVendor\WPDesk\WooCommerceShipping\ShippingMethod\Traits\LoggerTrait;
use UpsFreeVendor\WPDesk\WooCommerceShipping\ShippingMethod\Traits\SettingsTrait;
use UpsFreeVendor\WPDesk\WooCommerceShipping\ShippingMethod\Traits\ShippingServiceTrait;
/**
...
class ShippingMethod extends \WC_Shipping_Method
{
    ...
    {
        return new \UpsFreeVendor\WPDesk\WooCommerceShipping\ShippingBuilder\WooCommerceShippingMetaDataBuilder($this);
    }
    /**
     * Init form fields.
     */
    ...
    public function calculate_shipping($package = [])
    {
        if ($this->should_calculate_shipping()) {
            if ($this instanceof \UpsFreeVendor\WPDesk\WooCommerceShipping\ShippingMethod\HasCollectionPointFlatRate && $this->is_flat_rate_enabled($this)) {
                $this->add_rate_method(new \UpsFreeVendor\WPDesk\WooCommerceShipping\ShippingMethod\RateMethod\FlatRateRateMethod\CollectionPointFlatRateRateMethod($this->get_flat_rate_cost($this), $this->get_flat_rate_shipping_rate_suffix($this)));
            } else {
                $service = $this->get_shipping_service($this);
                $sender_address = $this->create_sender_address();
                if ($service instanceof \UpsFreeVendor\WPDesk\AbstractShipping\ShippingServiceCapability\CanRate) {
                    $this->add_rate_method(new \UpsFreeVendor\WPDesk\WooCommerceShipping\ShippingMethod\RateMethod\Standard\StandardServiceRateMethod($service));
                }
                if ($service instanceof \UpsFreeVendor\WPDesk\AbstractShipping\ShippingServiceCapability\CanRateToCollectionPoint) {
                    $this->add_rate_method(new \UpsFreeVendor\WPDesk\WooCommerceShipping\ShippingMethod\RateMethod\CollectionPoint\CollectionPointRateMethod($service));
                }
                $this->add_rate_method(new \UpsFreeVendor\WPDesk\WooCommerceShipping\ShippingMethod\RateMethod\Fallback\FallbackRateMethod('yes' === $this->get_option('debug_mode', 'no')));
            }
