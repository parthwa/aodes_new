...
class TrackerInitializer implements \UpsFreeVendor\WPDesk\PluginBuilder\Plugin\HookableCollection
{
     ...
     */
    public function __construct(string $plugin_file, string $plugin_slug, string $plugin_name, string $shop_url, \UpsFreeVendor\Octolize\Tracker\OptInNotice\ShouldDisplay $should_display)
    {
    ...
    public function hooks()
    {
        ...
        $this->add_hookable(\UpsFreeVendor\WPDesk\Tracker\Deactivation\TrackerFactory::createDefaultTracker($this->plugin_slug, $this->plugin_file, $this->plugin_name));
     ...
     */
    public static function create_from_plugin_info(\UpsFreeVendor\WPDesk_Plugin_Info $plugin_info, $should_display)
    {
        $shops = $plugin_info->get_plugin_shops();
        $shop_url = $shops[\get_locale()] ?? $shops['default'] ?? 'https://octolize.com';
        return new self($plugin_info->get_plugin_file_name(), $plugin_info->get_plugin_slug(), $plugin_info->get_plugin_name(), $shop_url, $should_display ?? new \UpsFreeVendor\Octolize\Tracker\OptInNotice\ShouldDisplayAlways());
    }
    /**
     ...
     * @return TrackerInitializer
     */
    public static function create_from_plugin_info_for_shipping_method(\UpsFreeVendor\WPDesk_Plugin_Info $plugin_info, string $shipping_method_id)
    {
        $shops = $plugin_info->get_plugin_shops();
        $shop_url = $shops[\get_locale()] ?? $shops['default'] ?? 'https://octolize.com';
