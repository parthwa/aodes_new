...
use FedExVendor\WPDesk\View\Renderer\SimplePhpRenderer;
use FedExVendor\WPDesk\View\Resolver\DirResolver;
class OptInPage implements \FedExVendor\WPDesk\PluginBuilder\Plugin\Hookable
{
    /**
     * @var string
     */
    ...
    public function wpdesk_tracker_page()
    {
        ...
        $shop = new \FedExVendor\WPDesk\Tracker\Shop($shop_url);
        $terms_url = $shop->get_usage_tracking_page();
        $logo = $shop->get_shop_logo_file();
        $logo_url = \plugin_dir_url(__FILE__) . '../../../assets/images/' . $logo;
        $renderer = new \FedExVendor\WPDesk\View\Renderer\SimplePhpRenderer(new \FedExVendor\WPDesk\View\Resolver\DirResolver(__DIR__ . '/views'));
        // WPCS: XSS ok.
