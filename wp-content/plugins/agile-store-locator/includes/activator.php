...
class Activator {
	...
	public static function add_basic_tables() {
		...
		//	Specials
		$sql = "CREATE TABLE IF NOT EXISTS `{$prefix}specials` (
			  `id` int unsigned NOT NULL AUTO_INCREMENT,
			  `name` varchar(255) DEFAULT NULL,
			  `ordr` int DEFAULT '0',
			  `lang` varchar(10) DEFAULT '',
		...
		//Alter Character SET
		//$sql = "ALTER TABLE {$prefix}categories CHARACTER SET UTF8MB4;";
		//$wpdb->query( $sql );
		...
		//	Update the country names 
			...
			$wpdb->update($prefix."countries", array('country' => 'Eswatini'), array('country' => 'Swaziland'));
		}
		...
		//	Specials
		$sql 	= "SELECT count(*) as c FROM information_schema.COLUMNS WHERE TABLE_NAME = '{$prefix}stores' AND COLUMN_NAME = 'special' AND TABLE_SCHEMA = '{$database}'";
		$result = $wpdb->get_results($sql);
		
		if($result[0]->c == 0) {
			$wpdb->query("ALTER TABLE {$prefix}stores ADD COLUMN `special` text;");
		}
