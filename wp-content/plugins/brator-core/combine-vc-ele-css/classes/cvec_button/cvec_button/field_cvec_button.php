 ...
 * MERCBRATORBILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
...
// Don't duplicate me!
if (!class_exists('ReduxFramework_cvec_button')) {

	/**
	 * Main ReduxFramework_cvec_button class
	 *
	 * @since       1.0.0
	 */
	class ReduxFramework_cvec_button
	{
		...
		public function __construct($field = array(), $value = '', $parent)
		{
			...
			$this->extension_dir = CVEC_FILE . 'classes/cvec_button/';
			$this->extension_url = CVEC_DIR_URL . 'classes/cvec_button/';
					...
					<tbody>
						<tr>
							...
							<fieldset id="<?php echo esc_attr($id); ?>-fieldset" class="redux-field redux-container-<?php echo esc_attr($this->field['type']) . ' ' . esc_attr($this->field['class']); ?>" data-id="<?php echo esc_attr($this->field['id']); ?>">
								<h3><?php esc_html_e('Combine VC Elementor CSS Option', 'redux-framework'); ?></h3>
								<div id="redux-object-browser"></div>
								<div id="redux-object-json" class="hide"><?php echo $json; ?></div>
								<a href="#" id="cvec_button_action" class="button"><?php esc_html_e('Clear Cache', 'redux-framework'); ?></a>
								</div>
							</fieldset>
							<?php
							if (true == $bDoClose) {
				...
				<?php
						...
						public function enqueue()
						{
							...
							wp_enqueue_script(
								...
								$this->extension_url . 'cvec_button/field_cvec_button' . Redux_Functions::isMin() . '.js',
								array('jquery'),
								ReduxFramework_extension_cvec_button::$version,
								true
							);

							wp_enqueue_style(
								'redux-options-object',
								$this->extension_url . 'cvec_button/field_cvec_button.css',
								array(),
								time(),
								'all'
							);
